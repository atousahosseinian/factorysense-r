...
def main():
    parser = argparse.ArgumentParser(
        description="Run single-image prediction with PatchCore-style baseline."
    )
    parser.add_argument("--model-path", default="models/patchcore_style_bottle.npz")
    parser.add_argument("--image-path", required=True)
    parser.add_argument("--device", default="auto")
    parser.add_argument("--output-dir", default="outputs/patchcore_style")
    args = parser.parse_args()

    model = PatchCoreStyleAnomalyDetector.load(
        args.model_path,
        device=args.device,
    )
    ...
    heatmap_path = output_dir / f"{image_path.stem}_patchcore_heatmap.png"
    overlay_path = output_dir / f"{image_path.stem}_patchcore_overlay.png"
    mask_path = output_dir / f"{image_path.stem}_patchcore_mask.png"
    ...
    print("PatchCore-style prediction result")
    print("=" * 40)
